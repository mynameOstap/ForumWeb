  ...
  return (
    <>
      <div className="bg-black min-h-screen p-4 text-white flex flex-col items-center ">
        <section className="flex flex-col items-center w-full max-w-[60%] mb-8">
          <img
          ...
          <div className="flex border-t border-b w-full border-gray-500 p-3 space-x-6 cursor-pointer relative">
            {TABS.map((tab) => (
              <div
                key={tab}
            ...
            {underline && (
              <span
                className={`absolute bottom-0 left-0 h-[2px] bg-white ${
                  underlineRef.current ? "transition-all duration-300 ease-in-out" : ""
                }`}
                style={{
        ...
        {activeTab === "Discussion" && (
          <div className="flex w-full max-w-[60%] gap-8">
            <div className="flex-1 flex flex-col">
              <div className="border border-white divide-y divide-white mb-6 ">
                <div
                  className="flex items-center gap-4 p-4 hover:bg-gray-950 cursor-pointer"
                  onClick={() => setOpenWriteMessage(true)}
                >
                   <img src={user?.avatarUrl ? `${API_BASE_URL}${user.avatarUrl}` : unknowImgWhite} className="w-[2rem] h-[2rem] rounded-full"/>
                  <input
                    type="text"
                    placeholder="Share something..."
                    className="flex-1 border border-white bg-transparent px-4 py-2 placeholder-gray-400"
                    readOnly
                  />
                  <button className="border border-white px-4 py-1 hover:bg-white hover:text-black">
                    Create Post
                  </button>
                </div>
